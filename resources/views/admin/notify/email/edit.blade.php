...
@section('head-tag')
    <title>بروز رسانی اطلاعیه ایمیلی</title>
                        ...
                        <section class="row">
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="select2 form-control form-control-sm" id="status" name="status">
                                        <option value="0" {{ old('status', $email->status) == 0 ? 'selected' : '' }}>
                                            غیرفعال</option>
                                        <option value="1" {{ old('status', $email->status) == 1 ? 'selected' : '' }}>
                                            فعال</option>
                                    </select>
                                </div>
                                @error('status')
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" class="alt-field-example-alt-field" name="published_at" {{ old('published_at', $email->published_at) }}/>
                                    <input type="text" class="form-control form-control-sm alt-field-example"
                                        id="published_at" value="{{ old('published_at', $email->published_at) }}">
                                </div>
                                @error('published_at')
                                    <div class="mt-1">
                                        <p class="text-danger">{{ $message }}</p>
