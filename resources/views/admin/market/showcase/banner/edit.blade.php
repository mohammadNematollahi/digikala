    ...
    <section class="row">
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section>
                    <form action="{{ route('admin.market.showcase.banner.update' , $banner->id) }}" method="POST"
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="position" id="position" class="form-control form-control-sm">
                                        @foreach ($position as $key => $value)
                                            <option value="{{ $key }}"
                                                {{ old('position' , $banner->position) == $key ? 'selected' : '' }}>{{ $value }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="select2 form-control form-control-sm" id="status" name="status">
                                        <option value="0" {{ old('status' , $banner->status) == 0 ? 'selected' : '' }}>غیرفعال</option>
