<section class="table-responsive">
    <table class="table table-striped table-hover">
        ...
        <tbody>
            ...
            @foreach ($email->files as $item)
                <tr>
                    <th>{{ $id }}</th>
                    <td>{{ $item->email->subject }}</td>
                    <td>{{ $item->email->id }}</td>
                    <td>{{ convertToMB($item->file_size) }}</td>
                    ...
                    <td class="width-16-rem text-left">
                        <a href="{{ route('admin.notify.email.all-files.edit', [$email->id , $item->id]) }}" class="btn btn-info btn-sm"><i
